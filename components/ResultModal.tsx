...
const ResultModal: React.FC<ResultModalProps> = ({ result, onClose }) => {
  ...
  const handleAudioTour = async () => {
    ...
    try {
        const audioBufferData = await generateAudioGuide(result.place_name, result.story);
        ...
        alert("Audio guide unavailable offline or API error.");
    } finally {
  ...
  return (
    <div className="fixed inset-0 z-[3000] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-md animate-fadeIn">
      <div className="w-full max-w-sm bg-white rounded-[32px] overflow-hidden shadow-2xl relative">
        ...
        {/* Status Light */}
        <div className={`absolute top-0 left-0 right-0 h-1.5 ${isSuccess ? 'bg-gradient-to-r from-emerald-400 to-teal-500' : 'bg-gray-300'}`}></div>

        <div className="pt-10 pb-8 px-8 text-center">
          <div className={`w-20 h-20 mx-auto mb-6 rounded-full flex items-center justify-center text-4xl shadow-lg ${isSuccess ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-400'}`}>
          ...
          {isSuccess && (
            <div className="bg-gradient-to-br from-yellow-50 to-orange-50 rounded-2xl p-4 mb-6 border border-orange-100">
              <p className="text-[10px] uppercase tracking-widest text-orange-400 font-bold mb-1">Earned</p>
              <p className="text-3xl font-black text-slate-800">+{result.points_earned} <span className="text-lg font-medium text-slate-400">pts</span></p>
            </div>
          )}

          <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 mb-6 text-left">
             <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Next Hint</p>
             <p className="text-sm font-semibold text-slate-700">{result.next_quest_hint}</p>
          </div>

          <button 
            onClick={onClose}
            className={`w-full py-4 rounded-2xl font-bold text-white shadow-xl transition-transform active:scale-95 ${isSuccess ? 'bg-slate-900 hover:bg-black' : 'bg-slate-400 hover:bg-slate-500'}`}
          >
