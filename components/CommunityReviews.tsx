...
const CommunityReviews: React.FC = () => {
  ...
  if (loading) return (
      <div className="flex gap-2 overflow-x-auto pb-4">
         {[1,2,3].map(i => (
             <div key={i} className="min-w-[200px] h-32 bg-slate-200 rounded-2xl animate-pulse"></div>
         ))}
      </div>
  );
  ...
  if (reviews.length === 0) return (
    <div className="text-center py-8 bg-white/50 rounded-3xl border border-dashed border-slate-300">
        ...
        <p className="text-slate-400 text-sm font-bold">No stories yet. Be the first!</p>
    </div>
  );
  ...
  return (
    <div className="grid grid-cols-1 gap-4">
      {reviews.map((review, index) => (
        <div key={index} className="group bg-white p-5 rounded-[24px] shadow-sm border border-slate-100 hover:shadow-md transition-all duration-300">
           ...
           <div className="flex justify-between items-start mb-3">
              <div className="flex items-center gap-3">
                 ...
                 <div>
                    <p className="text-sm font-bold text-slate-800 leading-tight">{review.userName || 'Anonymous'}</p>
                    <div className="flex items-center gap-1">
                        <span className="text-[10px] text-slate-400 font-medium">{review.date}</span>
                        <span className="text-[8px] text-emerald-500 bg-emerald-50 px-1 rounded font-bold">VERIFIED</span>
                    </div>
                 </div>
              </div>
              <div className="flex bg-amber-50 px-2 py-1 rounded-lg border border-amber-100">
                 <span className="text-xs font-bold text-amber-500 mr-1">{review.rating}.0</span>
